...
function _curlCommand(
  ...
  // `/petdesk` from inside their agent (or `petdesk hooks toggle`
...
function openCodePluginSource(): string {
  ...
  return String.raw`// petdex hook plugin. Auto-generated by 'petdesk hooks install'.
...
async function notify({ state, duration, text }) {
  ...
  // (or 'petdesk hooks toggle' from a shell). Bail before the token
