...
const html_tail =
    ...
    \\(() => {
    \\  const COLS = 8, ROWS = 9, MOOD_COLS = 6;
    \\  const moodSprites = (window.__PETDEX__ && window.__PETDEX__.moods) || {};
    \\  const STATES = {
    ...
    \\  function moodPos(c) { return `${c/(MOOD_COLS-1)*100}% 0%`; }
    ...
    \\  let currentFrame = { c: 0, r: 0 };
    \\  let stateTimer = null;
    \\  // Mood multiplies every frame's dwell. Updated by pollSidecarMood.
    \\  // Keeping the multiplier in JS (instead of reading the CSS var
    \\  // each tick) avoids a getComputedStyle call inside the animation
    \\  // loop — important on the 200ms poll cadence.
    \\  let moodAnimScale = 1;
    \\  function moodSpriteFor(state, mood) {
    \\    return state === 'idle' && typeof moodSprites[mood] === 'string' ? moodSprites[mood] : null;
    \\  }
    \\  function paintFrame(c, r) {
    \\    currentFrame = { c, r };
    \\    const moodUrl = moodSpriteFor(currentState, currentMood);
    \\    if (moodUrl) {
    \\      pet.style.backgroundImage = `url('${moodUrl}')`;
    \\      pet.style.backgroundSize = `${MOOD_COLS * 100}% 100%`;
    \\      pet.style.backgroundPosition = moodPos(c);
    \\      pet.dataset.moodSprite = '1';
    \\      return;
    \\    }
    \\    pet.style.backgroundImage = `url('spritesheet.webp')`;
    \\    pet.style.backgroundSize = `${COLS * 100}% ${ROWS * 100}%`;
    \\    pet.style.backgroundPosition = pos(c, r);
    \\    pet.dataset.moodSprite = '0';
    \\  }
    \\  function play(state) {
    ...
    \\    let i = 0;
    \\    paintFrame(frames[0].c, frames[0].r);
    \\    if (frames.length === 1) return;
    \\    const tick = () => {
    \\      stateTimer = setTimeout(() => {
    \\        i = (i + 1) % frames.length;
    \\        paintFrame(frames[i].c, frames[i].r);
    \\        tick();
    \\      }, Math.round(frames[i].d * moodAnimScale));
    \\    };
    \\    tick();
    \\  }
    \\  paintFrame(0, 0);
    ...
    \\
    \\  // Mood polling: mirrors state polling but writes to data-mood
    ...
    \\    if (!(window.zero && window.zero.invoke)) return;
    \\    try {
    \\      const r = await window.zero.invoke('petdex.read_runtime_mood', {});
    \\      if (!r || typeof r.counter !== 'number') return;
    \\      if (r.counter === lastMoodCounter) return;
...
const PetdexState = struct {
    ...
    bridge_handlers: [15]zero_native.BridgeHandler = undefined,
    ...
    fn bridge(self: *PetdexState) zero_native.BridgeDispatcher {
        self.bridge_handlers = .{
            ...
            .{ .name = "petdex.read_runtime_mood", .context = self, .invoke_fn = readRuntimeMoodCmd },
    ...
    // Reads ~/.petdex/runtime/mood.json (written by the sidecar's
    // POST /mood handler). Same shape as state.json: a monotonic
    // counter so the WebView can tell "new mood" from "same mood
    // re-served on every poll". Mood is independent of state — the
    // WebView composites them in CSS via data-state and data-mood.
    fn readRuntimeMoodCmd(context: *anyopaque, invocation: zero_native.bridge.Invocation, output: []u8) anyerror![]const u8 {
        _ = invocation;
        const self: *PetdexState = @ptrCast(@alignCast(context));
        const path = try std.fs.path.join(self.allocator, &.{ self.config_dir, "runtime", "mood.json" });
        defer self.allocator.free(path);
        var file = std.Io.Dir.openFileAbsolute(self.io, path, .{}) catch {
            return std.fmt.bufPrint(output, "{{\"fatigue\":0,\"level\":\"normal\",\"counter\":0}}", .{});
        };
        defer file.close(self.io);
        const stat = try file.stat(self.io);
        const size: usize = @intCast(stat.size);
    ...
    fn refreshPetsCmd(context: *anyopaque, invocation: zero_native.bridge.Invocation, output: []u8) anyerror![]const u8 {
        ...
        var mood_sprites = try loadMoodSpritesAcrossRoots(self.allocator, self.io, self.pets_roots, active_slug);
        defer {
            for (mood_sprites.items) |m| self.allocator.free(m.bytes);
            mood_sprites.deinit(self.allocator);
        }
        ...
        try writeMoodSpritesToAssetRoot(self.allocator, self.io, self.asset_root, mood_sprites.items);

        const petdex_json = try buildPetdexJson(self.allocator, pets.items, active_slug, mood_sprites.items);
...
const petdex_command_policies = [_]zero_native.BridgeCommandPolicy{
    ...
    .{ .name = "petdex.read_runtime_mood", .origins = &petdex_origins },
...
const MoodSpritePayload = struct { level: []const u8, bytes: []u8 };
const mood_levels = [_][]const u8{ "energetic", "normal", "tired", "exhausted", "dying" };
...
fn loadMoodSprites(allocator: std.mem.Allocator, io: std.Io, pets_dir: []const u8, slug: []const u8) !std.ArrayList(MoodSpritePayload) {
    var moods: std.ArrayList(MoodSpritePayload) = .empty;
    errdefer {
        for (moods.items) |m| allocator.free(m.bytes);
        moods.deinit(allocator);
    }
    ...
    inline for (mood_levels) |level| {
        const rel = try std.fs.path.join(allocator, &.{ "moods", level ++ ".webp" });
        defer allocator.free(rel);
        if (pet_dir.openFile(io, rel, .{})) |file| {
            defer file.close(io);
            try moods.append(allocator, .{
                .level = level,
                .bytes = try readFileAll(io, allocator, file, MAX_PET_BYTES),
            });
        } else |_| {}
    }
...
fn loadMoodSpritesAcrossRoots(allocator: std.mem.Allocator, io: std.Io, roots: []const []u8, slug: []const u8) !std.ArrayList(MoodSpritePayload) {
    for (roots) |root_path| {
        if (loadMoodSprites(allocator, io, root_path, slug)) |moods| {
            return moods;
        } else |_| {}
    }
    return .empty;
...
fn writeMoodSpritesToAssetRoot(allocator: std.mem.Allocator, io: std.Io, asset_root: []const u8, moods: []const MoodSpritePayload) !void {
    if (moods.len == 0) return;
    const moods_dir_path = try std.fs.path.join(allocator, &.{ asset_root, "moods" });
    defer allocator.free(moods_dir_path);
    try ensurePrivateDir(allocator, io, moods_dir_path);
    var moods_dir = try std.Io.Dir.openDirAbsolute(io, moods_dir_path, .{});
    defer moods_dir.close(io);
    for (moods) |m| {
        const name = try std.fmt.allocPrint(allocator, "{s}.webp", .{m.level});
        defer allocator.free(name);
        try writeFileAll(io, moods_dir, name, m.bytes);
    }
}
...
fn buildPetdexJson(allocator: std.mem.Allocator, pets: []const Pet, active_slug: []const u8, moods: []const MoodSpritePayload) ![]u8 {
    var buf: std.ArrayList(u8) = .empty;
    errdefer buf.deinit(allocator);
    ...
    try buf.appendSlice(allocator, "\",\"moods\":{");
    for (moods, 0..) |m, i| {
        if (i > 0) try buf.appendSlice(allocator, ",");
        try buf.appendSlice(allocator, "\"");
        try appendJsonEscaped(&buf, allocator, m.level);
        try buf.appendSlice(allocator, "\":\"moods/");
        try appendJsonEscaped(&buf, allocator, m.level);
        try buf.appendSlice(allocator, ".webp\"");
    }
    try buf.appendSlice(allocator, "}");
...
fn prepareAssetRoot(
    ...
    try writeMoodSpritesToAssetRoot(allocator, io, root, moods);
...
pub fn main(init: std.process.Init) !void {
    ...
    var mood_sprites = try loadMoodSprites(allocator, init.io, active_root, active_slug);
    defer {
        for (mood_sprites.items) |m| allocator.free(m.bytes);
        mood_sprites.deinit(allocator);
    }

    const petdex_json = try buildPetdexJson(allocator, pets.items, active_slug, mood_sprites.items);
    defer allocator.free(petdex_json);

    const html_doc = try buildHtml(allocator, petdex_json);
    defer allocator.free(html_doc);

    const asset_root = try prepareAssetRoot(allocator, init.io, config_dir, html_doc, sprite.ext, sprite.bytes, mood_sprites.items);
