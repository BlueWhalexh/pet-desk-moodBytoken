...
This fork adds token-driven mood rendering for local coding agents.

The sidecar periodically samples local agent usage files, converts usage
into a fatigue score, and writes `~/.petdex/runtime/mood.json`. The WebView
polls that file and, while the pet is idle, swaps the idle row to:
...
If a mood sprite is missing, the renderer falls back to the original
CSS-filter mood behavior.

Token calculation is intentionally configurable:
...
The scanner prefers native usage fields from local logs. When usage fields
are absent, it estimates transcript text with `js-tiktoken`; if that fails,
it falls back to `ceil(chars / 4)`. No prompt or response text leaves the
machine.
...
- Mood rows currently apply to idle only. Non-idle agent states keep using
  the base spritesheet rows.
