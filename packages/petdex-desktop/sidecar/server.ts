...
import { tokenMoodConfigFromEnv } from "./token-mood";
import {
...
const MOOD_PATH = join(RUNTIME_DIR, "mood.json");
...
const USAGE_MOOD_INTERVAL_MS = Number(
  process.env.PETDEX_USAGE_MOOD_INTERVAL_MS ?? 30_000,
);
const MANUAL_MOOD_HOLD_MS = Number(
...
let moodCounter = 0;
let lastManualMoodAt = 0;
function writeMood(payload: {
  fatigue: number;
  level: string;
  reason: string | null;
  ...
  writeFileSync(MOOD_PATH, JSON.stringify(out));
  return out;
}

function refreshMoodFromLocalUsage() {
  if (process.env.PETDEX_USAGE_MOOD === "0") return;
...
if (USAGE_MOOD_INTERVAL_MS > 0) {
  setTimeout(refreshMoodFromLocalUsage, 1000).unref();
  setInterval(refreshMoodFromLocalUsage, USAGE_MOOD_INTERVAL_MS).unref();
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    // Mood endpoints. Same auth + rate-limit story as /state and
    // /bubble: token gate keeps drive-by no-cors POSTs from the
    // browser out, the rate limiter stops a runaway monitor daemon
    // from spamming the file. Body shape:
    //   { fatigue?: number 0..1, level?: enum, reason?: string,
    //     agent_source?: string }
    // At least one of fatigue/level must be provided. If both are
    // present, fatigue wins (and we re-derive level so the value
    // on disk is internally consistent).
    if (req.method === "GET" && url.pathname === "/mood") {
      try {
        const raw = readFileSync(MOOD_PATH, "utf8");
        res.writeHead(200, {
          "content-type": "application/json",
    ...
    if (req.method === "POST" && url.pathname === "/mood") {
      const provided = req.headers[UPDATE_TOKEN_HEADER];
      const providedStr = Array.isArray(provided) ? provided[0] : provided;
      if (!providedStr || !constantTimeEquals(providedStr, UPDATE_TOKEN)) {
        return jsonResponse(res, 401, { ok: false, error: "unauthorized" });
      ...
      if (typeof data.fatigue === "number" && Number.isFinite(data.fatigue)) {
        fatigue = Math.min(1, Math.max(0, data.fatigue));
        level = fatigueToLevel(fatigue);
      } else if (
        typeof data.level === "string" &&
        MOOD_LEVEL_SET.has(data.level)
      ) {
        level = data.level;
        fatigue = levelToFatigue(level);
      } else {
        return jsonResponse(res, 400, {
          ok: false,
          error: "missing_fatigue_or_level",
          valid_levels: MOOD_LEVELS,
        });
      }
      const reason =
      ...
      const agentSource =
        typeof data.agent_source === "string"
          ? data.agent_source.slice(0, 64)
          : null;
      lastManualMoodAt = Date.now();
      if (fatigue === null || level === null) {
        return jsonResponse(res, 400, {
          ok: false,
          error: "missing_fatigue_or_level",
          valid_levels: MOOD_LEVELS,
      ...
      log(
        `mood fatigue=${result.fatigue.toFixed(2)} level=${result.level} src=${agentSource ?? "-"}`,
      );
